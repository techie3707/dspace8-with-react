...
const Search: React.FC = () => {
  ...
  const [size, setSize] = useState<number>(
    initialParams.size || resultsPerPageOptions[3].value
  );
  ...
  const [thumbnailsByItem, setThumbnailsByItem] = useState<
    Record<string, Bitstream[]>
  >({});
  const navigate = useNavigate();
  const [facetPagination, setFacetPagination] = useState<
    ...
    setFilters((prev) => {
      ...
      if (section.filterType === "boolean") {
        ...
        newValue = isChecked
          ? Array.from(
              new Map(
                [...(prev[filterType] || []), value].map((item) => [item, item])
              ).keys()
            )
          ...
          for (const result of searchResults) {
            ...
            if (bundles.length > 0) {
              const originalBundle =
                bundles.find((b) => b.name === "ORIGINAL") || bundles[0];
              const thumbnailBundle =
                bundles.find((b) => b.name === "THUMBNAIL") || bundles[0];
              const originalbitstreamsData = await fetchBitstreams(
                originalBundle.uuid
              );
              const thumbnailbitstreamsData = await fetchBitstreams(
                thumbnailBundle.uuid
              );
      ...
      case "checkbox":
        ...
        return (
          <ul style={{ listStyle: "none", padding: "0" }}>
                ...
                >
                  <div style={{ display: "flex", alignItems: "center" }}>
                    <input
                      ...
                      onChange={(e) =>
                        updateFilter(section.id, option.id, e.target.checked)
                      }
                    />
            ...
            {/* Show more button */}
            {facets[section.id].length % facetPagination[section.id]?.size ===
              0 && (
              <button
              ...
              >
                <div style={{ display: "flex", alignItems: "center" }}>
                  <input
                    ...
                    onChange={(e) =>
                      updateFilter(section.id, true, e.target.checked)
                    }
                  />
      ...
      <div className="filters-and-results">
        <div className="filters-and-setting">
          <div className="filters col-3">
            <div className="Zns0ac">
              <span className="I75YIf">Filter by</span>
            </div>
            {filterSections.map((section) => {
              const shouldShowSection =
                section.filterType === "range" ||
                (section.filterType === "checkbox" &&
                  facets[section.id]?.length > 0) ||
                (section.filterType === "boolean" &&
                  hasFileCounts.hasFileCount > 0);
              if (!shouldShowSection) return null;
              return (
                <div key={section.id}>
                  <div
                    className={`filter_name ${
                      expandedSections[section.id] ? "" : "border-bottom"
                    }`}
                  >
                    <h2 className="ZF0dQe">{section.label}</h2>
                    <button
                      className={`toggle-button ${
                        expandedSections[section.id] ? "up" : "down"
                      }`}
          ...
          <div className="dropdown-container">
            <h1 className="Zns0ac">
              <span className="I75YIf">Setting</span>
            </h1>
            <div>
              ...
              <Grid item xs={8.5} sm={8.5} lg={11}>
                {loadingTime && (
                  <h4 style={{ margin: "0px" }}>
                    ...
                    {Math.max(
                      parseFloat(loadingTime) > 0.8
                        ? parseFloat(loadingTime) - 0.5
            ...
            <Grid container spacing={2} className="results-body">
              {viewMode === "list"
                ? searchResults.map((result, index) => {
                    ...
                    const title =
                      metadata?.["dc.title"]?.[0]?.value || "Unknown Title";
                    const uuid = result._embedded?.indexableObject?.uuid;
                    const abstract =
                      ...
                      <Grid item xs={12} key={index}>
                        ...
                        >
                          {thumbnailsByItem[
                            ...
                            ?.filter((bitstream) =>
                              /\.(jpe?g|png)$/i.test(bitstream.name)
                            )
                            .slice(0, 1)
                            .map((bitstream) => (
                              <img
                ...
                : searchResults.map((result, index) => {
                    const metadata =
                      result._embedded?.indexableObject?.metadata;
                    const type = result._embedded?.indexableObject?.type;
                    const title = getMetadataValue(
                      metadata,
                      metadataFields.title
                    );
                    const uuid = result._embedded?.indexableObject?.uuid;
                    const abstract = getMetadataValue(
                      metadata,
                      metadataFields.abstract
                    );
                    const date = getMetadataValue(
                      metadata,
                      metadataFields.date
                    );
                    const author = getMetadataValue(
                      metadata,
                      metadataFields.author
                    );
                    const entity = getMetadataValue(
                      metadata,
                      metadataFields.entityType
                    );
                    const publisher = getMetadataValue(
                      metadata,
